		 	     ...
		 	     :command (lambda ()
					(let* ((pdf-path (get-save-file :filetypes '(("PDF" ".pdf")))))
					  (unless (equal "" pdf-path)
					    ...
					    (setq *school-info-main-frame* (make-instance 'frame :borderwidth 5 :relief :ridge))
					    (grid *school-info-main-frame* 0 0)
					    (grid (make-instance 'label :master *school-info-main-frame* :text "The subjects have been exported to pdf") 1 0))
					  ))))
(defun get-paper-data ()
  "get subject data as (class-stream stream-subjects pape)"
  (labels ((stream-subjects (stream-id) (remove-if #'null (mapcar #'cadr (get-subjects stream-id))))
    ...
    (let ((classes (get-classes)))
      (mapcar (lambda (class)
		(let* ((class-id (car class))
		  ...
		  (remove-if #'null (list (cadr class)
					  (remove-if #'null streams)
					  (remove-if #'null (mapcar #'stream-subjects stream-ids))
					  (remove-if #'null (mapcar (lambda (stream-id)
								      (remove-if #'null (mapcar #'cadr
												(remove-if #'null (mapcar (lambda (subject-id)
															    (get-papers subject-id))
															  (remove-if #'null (mapcar #'car (get-subjects stream-id))))))))
								    stream-ids))))))
	      classes))))
		 	     ...
		 	     :command (lambda ()
					(let* ((pdf-path (get-save-file :filetypes '(("PDF" ".pdf")))))
					  (unless (equal "" pdf-path)
					    ...
					    (setq *school-info-main-frame* (make-instance 'frame :borderwidth 5 :relief :ridge))
					    (grid *school-info-main-frame* 0 0)
					    (grid (make-instance 'label :master *school-info-main-frame* :text "The papers have been exported to pdf") 1 0))
					  ))))
