...
;; SCHOOL DETAILS
(defun save-school-details (name motto location phone-number pobox fax email logo-path)
  (conn
    (dolist (detail (list (cons "name" name)
			  ...
			  (cons "email" email)
			  (cons "logo" logo-path)))
      (execute-non-query db "insert or replace into school_details (detail_name, detail) values (?, ?)" (car detail) (cdr detail)))))

(defun get-school-details ()
  (conn (execute-to-list db "select detail_name, detail from school_details")))

(defun school-details-p ()
  "check if any of the details required to make pdf headers is missing, in which case, you return nil, or else return true."
  (let* ((details (get-school-details))
	 (data (mapcar (lambda (detail) (find detail details :key #'car :test #'equal)) '("name" "location" "phone_number" "pobox" "email" "logo"))))
    (if (member nil data) nil t)))
...
(defun alert-missing-details ()
  (make-response *missing-details-message*))
			       ...
			       :command (lambda ()
					  (if (school-details-p)
					      (let ((pdf-path (get-save-file :filetypes '(("PDF" ".pdf"))))
						    (level-data (mapcar (lambda (level) (cl:last level)) levels)))
						(unless (equal "" pdf-path)
						  (generate-pdf (:file-path pdf-path
								 :table-title "Levels"
								 :table-headings '("Levels")
								 :table-data level-data))
						  (make-response "The levels have been exported to pdf")
						  ))
...
(defun get-stream-data ()
  ...
  (let ((levels (|get-level|)))
    (labels ((get-level-streams (level-id)
      ...
      (mapcar (lambda (level) (remove-if #'null (list (cadr level)
						      (remove-if #'null (mapcar #'cadr (get-classes (car level))))
						      (remove-if #'null (get-level-streams (car level)))))) levels))))
  ...
  (make-instance 'menubutton :master stream-menu :text "Export to PDF"
		 	     :command (lambda ()
					(if (school-details-p)
						...
						(make-response "The streams have been exported to pdf")
						)
					      )
					    (alert-missing-details)))))
...
(defun get-subject-data ()
  ...
  (let ((classes (get-classes)))
    (mapcar (lambda (class)
		...
		(remove-if #'null (list (cadr class)
					(remove-if #'null (mapcar #'cadr streams))
					(remove-if #'null  (mapcar (lambda (stream-subjects)
  ...
  (make-instance 'menubutton :master subject-menu :text "Export to PDF"
		 	     :command (lambda ()
					(if (school-details-p)
						...
						(make-response "The subjects have been exported to pdf")
						)
					      )
					    (alert-missing-details)))))
(defun get-paper-data ()
  "get subject data as (class stream stream-subjects papers)"
  (labels ((remove-null (data) (remove-if #'null data))
	   (stream-subjects (stream-id) (remove-null (mapcar #'cadr (get-subjects stream-id))))
	   (stream-subject-ids (stream-id) (remove-null (mapcar #'car (get-subjects stream-id))))
	   (streams (class-id) (mapcar #'cadr (get-streams class-id)))
	   (subject-papers (subject-id) (mapcar #'cadr (get-papers subject-id))))
    (let ((classes (get-classes)))
      (mapcar (lambda (class)
		(let* ((class-id (car class))
		       ...
		       papers-list)
		  (remove-null (list (cadr class)
				     (remove-null streams)
				     (remove-null (mapcar #'stream-subjects stream-ids))
				     (dolist (stream-id stream-ids)
				       (dolist (subject-id (stream-subject-ids stream-id))
					 (setq papers-list (append papers-list (list (subject-papers subject-id))))))
				     (remove-null papers-list)
				     ))))
	      classes))))
  ...
  (make-instance 'menubutton :master paper-menu :text "Export to PDF"
		 	     :command (lambda ()
					(if (school-details-p)
						...
						(generate-pdf (:file-path pdf-path
							       :table-title "Subjects by Stream"
							       :table-headings '("Class" "Stream" "Subject" "Paper")
							       :table-data (get-paper-data)))
						(make-response "The papers have been exported to pdf"))
					      )
					    (alert-missing-details)))))
		 	       ...
		 	       :command (lambda ()
					  (if (school-details-p)
						...
						(unless (equal "" pdf-path)
						  (generate-pdf (:file-path pdf-path
								 :table-title "Houses"
								 :table-headings '("Houses")
								 :table-data (mapcar (lambda (house) (list (cadr house))) houses)))
						  (make-response "The houses have been exported to pdf")
						  ))
					      (alert-missing-details))))))
    ...
    (make-instance 'menubutton :master dorm-menu :text "Export to PDF"
		 	       :command (lambda ()
					  (if (school-details-p)
						...
						(unless (equal "" pdf-path)
						  (generate-pdf (:file-path pdf-path
								 :table-title "Dormitories"
								 :table-headings '("Dormitories")
								 :table-data (mapcar (lambda (dorm) (list (cadr dorm))) dorms)))
						  (make-response "The dormitories have been exported to pdf")
						  ))
					      (alert-missing-details))))))
		 	     ...
		 	     :command (lambda ()
					(if (school-details-p)
					    (let* ((pdf-path (get-save-file :filetypes '(("PDF" ".pdf")))))
					      (unless (equal "" pdf-path)
						(generate-pdf (:file-path pdf-path
							       :table-title "School Details"
							       :table-headings '("Detail" "Value")
							       :table-data (get-school-details)))
						(make-response "The school details have been exported to pdf")
						))
					    (alert-missing-details)))))
...
(defun start ()
  "start the info application, try to create the tables, bind the error to continue execution if the tables are already present. enable foreign key support on the db"
  (conn
...
(defun details-form ()
  ...
  (flet ((get-detail (data key) (cadr (assoc key data :test #'string-equal))))
    (let* ((data (get-school-details))
	   (heading (make-instance 'label :master *main-frame* :text "Basic Information about the school."))
	   (name-label (make-instance 'label :master *main-frame* :text "Name"))
	   (name-entry (make-instance 'entry :master *main-frame* :text (get-detail data "name")))
	   (motto-label (make-instance 'label :master *main-frame* :text "Motto"))
	   (motto-entry (make-instance 'entry :master *main-frame* :text (get-detail data "motto")))
	   (location-label (make-instance 'label :master *main-frame* :text "Location"))
	   (location-entry (make-instance 'entry :master *main-frame* :text (get-detail data "location")))
	   (phone-number-label (make-instance 'label :master *main-frame* :text "Phone Number"))
	   (phone-number-entry (make-instance 'entry :master *main-frame* :text (get-detail data "phone_number")))
	   (pobox-label (make-instance 'label :master *main-frame* :text "P.O. Box"))
	   (pobox-entry (make-instance 'entry :master *main-frame* :text (get-detail data "pobox")))
	   (fax-label (make-instance 'label :master *main-frame* :text "Fax"))
	   (fax-entry (make-instance 'entry :master *main-frame* :text (get-detail data "fax")))
	   (email-label (make-instance 'label :master *main-frame* :text "Email"))
	   (email-entry (make-instance 'entry :master *main-frame* :text (get-detail data "email")))
	   (logo-label (make-instance 'label :master *main-frame* :text "Logo"))
	   (logo-entry (make-instance 'entry :master *main-frame* :text (get-detail data "logo")))
	   (logo-file-button (make-instance 'button :master *main-frame* :text "Select logo image"
						    :command (lambda ()
						               (let* ((logo-path (get-open-file :filetypes '(("JPEG" ".jpeg")))))
								 (unless (equal "" logo-path)
								   (setf (text logo-entry) logo-path))))))
	   (save-button (make-instance 'button :master *main-frame*
					       :text "Save School Details"
					       :command (lambda ()
							  (unless (or (null (get-detail data "logo")) (equal (text logo-entry) (get-detail data "logo")))
							    (handler-case (delete-file *logo-path*)
							      (sb-ext:delete-file-error (err)
								(declare (ignore err)))))
							  (uiop:copy-file (text logo-entry) *logo-path*)
      ...
      (grid  logo-label 8 0 :padx 10 :pady 5)
      (grid  logo-file-button 8 2 :padx 10 :pady 5)
      (grid  logo-entry 8 3 :padx 10 :pady 5)
      (grid save-button 9 2 :pady 10))))
...
(defun show-details ()
  ...
  (flet ((get-detail (data key) (cadr (assoc key data :test #'string-equal))))
    (let* ((data (get-school-details))
	   (heading (make-instance 'label :master *main-frame* :text "Basic Information about the school."))
	   (name-label (make-instance 'label :master *main-frame* :text "Name"))
	   (name-data (make-instance 'label :master *main-frame* :text (get-detail data "name")))
	   (motto-label (make-instance 'label :master *main-frame* :text "Motto"))
	   (motto-data (make-instance 'label :master *main-frame* :text (get-detail data "motto")))
	   (location-label (make-instance 'label :master *main-frame* :text "Location"))
	   (location-data (make-instance 'label :master *main-frame* :text (get-detail data "location")))
	   (phone-number-label (make-instance 'label :master *main-frame* :text "Phone Number"))
	   (phone-number-data (make-instance 'label :master *main-frame* :text (get-detail data "phone_number")))
	   (pobox-label (make-instance 'label :master *main-frame* :text "P.O. Box"))
	   (pobox-data (make-instance 'label :master *main-frame* :text (get-detail data "pobox")))
	   (fax-label (make-instance 'label :master *main-frame* :text "Fax"))
	   (fax-data (make-instance 'label :master *main-frame* :text (get-detail data "fax")))
	   (email-label (make-instance 'label :master *main-frame* :text "Email"))
      ...
      (grid logo-label 8 0 :padx 10 :pady 5)
      (grid logo-data 8 2 :padx 10 :pady 5))))
...
(defun level-form (&optional level)
  ...
  (let* ((index 1)
	 ...
	 (level-number-label (make-instance 'label :master *main-frame* :text "Enter number of levels"))
	 (level-number-entry (make-instance 'entry :master *main-frame* :text "1"))
	 (level-label (make-instance 'label :master *main-frame* :text "Enter Level Name"))
	 (level-entry (make-instance 'entry :master *main-frame* :text (cadr level)))
	 (save-button (make-instance 'button :master *main-frame*
					     :text "Save Level(s)" :command (let ((message "The level(s) has been saved")) 
										...
										(make-response message)))))
	 (level-number-button (make-instance 'button :master *main-frame* :text "Change number"
						     :command (lambda ()
					...
					; run this code everytime, because you need to remove the unwanted entries from the entries list
									...
									(t
									 (loop for i from 1 to (- number-of-levels (length level-entries)) ; subtract the entries already present
									       do (let ((new-level-label (make-instance 'label :master *main-frame* :text "Enter Level Name"))
											(new-level-entry (make-instance 'entry :master *main-frame*)))
...
(defun show-levels ()
  ...
  (let ((index 2)
	...
	(grid (make-instance 'label :master *main-frame* :text "No level has been added yet.") 1 0 :padx 10 :pady 5)
	(dolist (level levels)
	  (grid (make-instance 'label :master *main-frame* :text (cadr level)) index 0)
	  (setq index (+ index 1))))))
...
(defun house-form (&optional house)
  ...
  (let* ((index 1)
	 ...
	 (house-number-label (make-instance 'label :master *main-frame* :text "Enter number of houses"))
	 (house-number-entry (make-instance 'entry :master *main-frame* :text "1"))
	 (house-label (make-instance 'label :master *main-frame* :text "Enter House Name"))
	 (house-entry (make-instance 'entry :master *main-frame* :text (cadr house)))
	 (save-button (make-instance 'button :master *main-frame*
					     :text "Save House(s)" :command (let ((message "The house(s) has been saved"))
										...
										(make-response message)))))
	 (house-number-button (make-instance 'button :master *main-frame* :text "Change number"
						     :command (lambda ()
								  ...
								  (cond ((eq 0 number-of-houses) ())
									...
									(t
									 (loop for i from 1 to (- number-of-houses (length house-entries)) ;subtract those already present
									       do (let ((new-house-label (make-instance 'label :master *main-frame* :text "Enter House Name"))
											(new-house-entry (make-instance 'entry :master *main-frame*)))
...
(defun show-houses ()
  ...
  (let ((index 2)
	...
	(grid (make-instance 'label :master *main-frame* :text "No house has been added yet") 1 0 :padx 10 :pady 5)
	(dolist (house houses)
	  (grid (make-instance 'label :master *main-frame* :text (cadr house)) index 0)
	  (setq index (+ index 1))))))
...
(defun dorm-form (&optional dorm)
  ...
  (let* ((index 1)
	 ...
	 (dorm-number-label (make-instance 'label :master *main-frame* :text "Enter number of dorms"))
	 (dorm-number-entry (make-instance 'entry :master *main-frame* :text "1"))	
	 (dorm-label (make-instance 'label :master *main-frame* :text "Enter Dorm Name"))
	 (dorm-entry (make-instance 'entry :master *main-frame* :text (cadr dorm)))
	 (save-button (make-instance 'button :master *main-frame*
					     :text "Save Dorm(s)" :command (let ((message "The dorm(s) has been saved"))
									     (lambda ()
									       ...
									       (make-response message)))))
	 (dorm-number-button (make-instance 'button :master *main-frame* :text "Change number"
						    :command (lambda ()
								 ...
								 (cond ((eq 0 number-of-dorms) ())
									...
									(loop for i from 1 to (- number-of-dorms (length dorm-entries))
									      do (let ((new-dorm-label (make-instance 'label :master *main-frame* :text "Enter Dorm Name"))
										       (new-dorm-entry (make-instance 'entry :master *main-frame*)))
...
(defun show-dorms ()
  ...
  (let ((index 2)
	...
	(grid (make-instance 'label :master *main-frame* :text "No dorm has been added yet.") 1 0 :padx 10 :pady 5)
	(dolist (dorm dorms)
	  (grid (make-instance 'label :master *main-frame* :text (cadr dorm)) index 0)
	  (setq index (+ index 1))))))
...
(defun class-form (&optional level class)
  ...
  (let ((levels (|get-level|)))   
	...
	(let* ((index 1)
	       ...
	       (class-number-label (make-instance 'label :master *main-frame* :text "Enter number of classes"))
	       (class-number-entry (make-instance 'entry :master *main-frame* :text "1"))	
	       (level-label (make-instance 'label :master *main-frame* :text "Select Level"))
	       (level-combobox (make-instance 'combobox :text (if level (cadr level)
								  (cadar levels)) :master *main-frame* :values (mapcar (lambda (x) (cadr x)) levels)))
	       (class-label (make-instance 'label :master *main-frame* :text "Enter Class Name"))
	       (class-entry (make-instance 'entry :master *main-frame* :text (cadr class)))
	       (save-button (make-instance 'button :master *main-frame*
						   :text "Save Class(es)" :command (let ((message "The class(es) has been saved"))
										     (lambda ()
										       ...
										       (make-response message)))))
	       (class-number-button (make-instance 'button :master *main-frame* :text "Change number"
							   :command (lambda ()
										 ...
										 (setq class-entries (cdr class-entries)))
									      ...
									      (t
									       (loop for i from 1 to (- number-of-classes (length class-entries))
										     do (let ((new-class-label (make-instance 'label :master *main-frame* :text "Enter Class Name"))
											      (new-class-entry (make-instance 'entry :master *main-frame*)))
...
display the levels and classes in a grid, use x-pos and y-pos to control the location in the grid"
  ...
  (grid (make-instance 'label :master *main-frame* :text "Classes") 0 1)
  (grid (make-instance 'separator :master *main-frame*) 1 0 :sticky "nsew" :columnspan 2)
  (let* ((levels (|get-level|))
	 (r 2) ; start from row 2
	 )
    (dolist (level levels)
      (grid (make-instance 'label :master *main-frame* :text (cadr level)) r 0) ; all levels go into column 0 
      (let ((classes (get-classes (car level))))
	(dolist (class classes)
	  (grid (make-instance 'label :master *main-frame* :text (cadr class)) r 1); all classes go into column 1
	  (setq r (+ 1 r))
	  (unless (equal (car (cl:last classes)) class)
	    (grid (make-instance 'separator :master *main-frame*) r 1 :sticky "nsew" :columnspan 2))
	  (setq r (+ r 1))))
      (unless (equal (car (cl:last levels)) level)
	(grid (make-instance 'separator :master *main-frame*) r 0 :sticky "nsew" :columnspan 2))
      (setq r (+ r 1)))
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 1 :sticky "nsw" :rowspan r)))
...
(defun stream-form (class-data &optional stream-data)
  ...
  (let ((classes (get-classes))
	...
	(let* ((index 1)
	       ...
	       (stream-number-label (make-instance 'label :master *main-frame* :text "Enter number of streams"))
	       (stream-number-entry (make-instance 'entry :master *main-frame* :text "1"))	
	       
	       (class-label (make-instance 'label :master *main-frame* :text "Choose Class Name"))
	       (class-combobox (make-instance 'entry :text class-text :master *main-frame* :state :disabled))
	       (stream-label (make-instance 'label :master *main-frame* :text "Enter Stream Name"))
	       (stream-entry (make-instance 'entry :master *main-frame* :text stream-text))
	       (save-button (make-instance 'button :master *main-frame*
						   :text "Save Stream(s)"
						   :command (let ((message "The stream(s) has been saved"))
								...
								(make-response message)))))
	       (stream-number-button (make-instance 'button :master *main-frame* :text "Change number"
							    :command (lambda ()
									 ...
									 (cond ((eq 0 number-of-streams) ())
										...
										(loop for i from 1 to (- number-of-streams (length stream-entries))
										      do (let ((new-stream-label (make-instance 'label :master *main-frame* :text "Enter Stream Name"))
											       (new-stream-entry (make-instance 'entry :master *main-frame*)))
...
(defun show-streams ()
  ...
  (grid (make-instance 'label :master *main-frame* :text "Level") 0 0)
  (grid (make-instance 'label :master *main-frame* :text "Classes") 0 1)
  (grid (make-instance 'label :master *main-frame* :text "Streams") 0 2)
  (grid (make-instance 'separator :master *main-frame*) 1 0 :columnspan 3 :sticky "nsew")
  (let ((r 2)
	(levels (|get-level|)))
    (dolist (level levels)
      (grid (make-instance 'label :master *main-frame* :text (cadr level)) r 0) ; all levels go into column 0 
	...
	(dolist (class classes)
	  ...
	  (let ((streams (get-streams (car class))))
	    (dolist (stream streams)
	      (grid (make-instance 'label :master *main-frame* :text (cadr stream)) r 2)
	      (setq r (+ 1 r))
	      (unless (equal (car (cl:last streams)) stream)
		(grid (make-instance 'separator :master *main-frame*) r 2 :sticky "nsew" :columnspan 1))
	      (setq r (+ 1 r))))
	  (setq r (+ 1 r))
	  (unless (equal (car (cl:last classes)) class)
	    (grid (make-instance 'separator :master *main-frame*) r 1 :sticky "nsew" :columnspan 2))
	  (setq r (+ r 1))))
      (unless (equal (car (cl:last levels)) level)
	(grid (make-instance 'separator :master *main-frame*) r 0 :sticky "nsew" :columnspan 3))
      (setq r (+ r 1)))
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 1 :sticky "nsw" :rowspan r)
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 2 :sticky "nsw" :rowspan r)
    ))
...
(defun subject-form (class-data stream-data &optional subject-data)
  ...
  (let ((class (cadr class-data))
    ...
    (let* ((index 1)
	   ...
	   (subject-number-label (make-instance 'label :master *main-frame* :text "Enter number of subjects"))
	   (subject-number-entry (make-instance 'entry :master *main-frame* :text "1"))	
	   (class-label (make-instance 'label :master *main-frame* :text "Class"))
	   (class-entry (make-instance 'entry :master *main-frame* :text class :state :disabled))
	   (stream-label (make-instance 'label :master *main-frame* :text "Stream"))
	   (stream-entry (make-instance 'entry :master *main-frame* :text stream :state :disabled))
	   (subject-label (make-instance 'label :master *main-frame* :text "Subject Name"))
	   (subject-entry (make-instance 'entry :master *main-frame* :text subject-name))
	   (subject-code-label (make-instance 'label :master *main-frame* :text "Subject Code"))
	   (subject-code-entry (make-instance 'entry :master *main-frame* :text subject-code))
	   (save-button (make-instance 'button :master *main-frame*
					       :text "Save Subject(s)"
					       :command (let ((message "The subject(s) has been saved."))
							  (lambda ()
							    ...
							    (make-response message)))))
	   (subject-number-button (make-instance 'button :master *main-frame* :text "Change number"
							 :command (lambda ()
								      ...
								      (cond ((eq 0 number-of-subjects) ())
									    ...
									    (t
									     (loop for i from 1 to (- number-of-subjects (length subject-entries))
										   do (let ((new-subject-label (make-instance 'label :master *main-frame* :text "Subject Name"))
											    (new-subject-entry (make-instance 'entry :master *main-frame*))
											    (new-subject-code-label (make-instance 'label :master *main-frame* :text "Subject Code"))
											    (new-subject-code-entry (make-instance 'entry :master *main-frame*)))
...
(defun show-subjects ()
  ...
  (grid (make-instance 'label :master *main-frame* :text "Classes") 0 1)
  (grid (make-instance 'label :master *main-frame* :text "Streams") 0 2)
  (grid (make-instance 'label :master *main-frame* :text "Subjects") 0 3)
  (grid (make-instance 'separator :master *main-frame*) 1 0 :columnspan 4 :sticky "nsew")
  (let ((r 2)
	(levels (|get-level|)))
    (dolist (level levels)
      (grid (make-instance 'label :master *main-frame* :text (cadr level)) r 0) ; all levels go into column 0 
	...
	(dolist (class classes)
	  ...
	  (let ((streams (get-streams (car class))))
	    (dolist (stream streams)
	      ...
	      (let ((subjects (get-stream-subjects (car stream))))
		(dolist (subject subjects)
		  (grid (make-instance 'label :master *main-frame* :text (cadr subject)) r 3)
		  (setq r (+ 1 r))
		  (unless (equal (car (cl:last subjects)) subject)
		    (grid (make-instance 'separator :master *main-frame*) r 3 :sticky "nsew" :columnspan 1))
		  (setq r (+ 1 r))))
	      (setq r (+ 1 r))
	      (unless (equal (car (cl:last streams)) stream)
		(grid (make-instance 'separator :master *main-frame*) r 2 :sticky "nsew" :columnspan 2))
	      (setq r (+ 1 r))))
	  (setq r (+ 1 r))
	  (unless (equal (car (cl:last classes)) class)
	    (grid (make-instance 'separator :master *main-frame*) r 1 :sticky "nsew" :columnspan 3))
	  (setq r (+ r 1))))
      (unless (equal (car (cl:last levels)) level)
	(grid (make-instance 'separator :master *main-frame*) r 0 :sticky "nsew" :columnspan 4))
      (setq r (+ r 1)))
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 1 :sticky "nsw" :rowspan r)
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 2 :sticky "nsw" :rowspan r)
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 3 :sticky "nsw" :rowspan r)))
...
(defun paper-form (class-data stream-data subject-data &optional paper-data)
  ...
  (let ((subject-id (car subject-data))
    ...
    (let* ((index 1)
	   ...
	   (paper-number-label (make-instance 'label :master *main-frame* :text "Enter number of papers"))
	   (paper-number-entry (make-instance 'entry :master *main-frame* :text "1"))
	   (class-label (make-instance 'label :master *main-frame* :text "Class"))
	   (class-entry (make-instance 'entry :master *main-frame* :text (cadr class-data) :state :disabled))
	   (stream-label (make-instance 'label :master *main-frame* :text "Stream"))
	   (stream-entry (make-instance 'entry :master *main-frame* :text (cadr stream-data) :state :disabled))
	   (subject-label (make-instance 'label :master *main-frame* :text "Subject"))
	   (subject-entry (make-instance 'entry :master *main-frame* :text subject-name :state :disabled))
	   (paper-label (make-instance 'label :master *main-frame* :text "Paper Name"))
	   (paper-entry (make-instance 'entry :master *main-frame* :text paper-name))
	   (paper-code-label (make-instance 'label :master *main-frame* :text "Paper Code"))
	   (paper-code-entry (make-instance 'entry :master *main-frame* :text paper-name))
	   (save-button (make-instance 'button :master *main-frame*
					       :text "Save Paper(s)"
					       :command (let ((message "The paper(s) has been saved."))
							  (lambda ()
							    ...
							    (make-response message)))))
	   (paper-number-button (make-instance 'button :master *main-frame* :text "Change number"
						       :command (lambda ()
								    ...
								    (cond ((eq 0 number-of-papers) ())
									  ...
									  (t
									   (loop for i from 1 to (- number-of-papers (length paper-entries))
										 do (let ((new-paper-label (make-instance 'label :master *main-frame* :text "Paper Name"))
											  (new-paper-entry (make-instance 'entry :master *main-frame*))
											  (new-paper-code-label (make-instance 'label :master *main-frame* :text "Paper Code"))
											  (new-paper-code-entry (make-instance 'entry :master *main-frame*)))
...
(defun show-papers ()
  ...
  (grid (make-instance 'label :master *main-frame* :text "Streams") 0 2)
  (grid (make-instance 'label :master *main-frame* :text "Subjects") 0 3)
  (grid (make-instance 'label :master *main-frame* :text "Papers") 0 4)
  (grid (make-instance 'separator :master *main-frame*) 1 0 :columnspan 5 :sticky "nsew")
  (let ((r 2)
	(levels (|get-level|)))
    (dolist (level levels)
      (grid (make-instance 'label :master *main-frame* :text (cadr level)) r 0) ; all levels go into column 0 
	...
	(dolist (class classes)
	  ...
	  (let ((streams (get-streams (car class))))
		...
		(dolist (subject subjects)
		  ...
		  (let ((papers (get-papers (car subject))))
		    (dolist (paper papers)
		      (grid (make-instance 'label :master *main-frame* :text (cadr paper)) r 4)
		      (setq r (+ 1 r))
		      (unless (equal (car (cl:last papers)) paper)
			(grid (make-instance 'separator :master *main-frame*) r 4 :sticky "nsew" :columnspan 1))
		      (setq r (+ 1 r))))
		  (setq r (+ 1 r))
		  (unless (equal (car (cl:last subjects)) subject)
		    (grid (make-instance 'separator :master *main-frame*) r 3 :sticky "nsew" :columnspan 2))
		  (setq r (+ 1 r))))
	      (setq r (+ 1 r))
	      (unless (equal (car (cl:last streams)) stream)
		(grid (make-instance 'separator :master *main-frame*) r 2 :sticky "nsew" :columnspan 3))
	      (setq r (+ 1 r))))
	  (setq r (+ 1 r))
	  (unless (equal (car (cl:last classes)) class)
	    (grid (make-instance 'separator :master *main-frame*) r 1 :sticky "nsew" :columnspan 4))
	  (setq r (+ r 1))))
      (unless (equal (car (cl:last levels)) level)
	(grid (make-instance 'separator :master *main-frame*) r 0 :sticky "nsew" :columnspan 5))
      (setq r (+ r 1)))
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 1 :sticky "nsw" :rowspan r)
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 2 :sticky "nsw" :rowspan r)
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 3 :sticky "nsw" :rowspan r)
    (grid (make-instance 'separator :master *main-frame* :orientation "vertical") 0 4 :sticky "nsw" :rowspan r)))
