...
(defparameter *logo-path* (uiop:native-namestring #p"static/logo.jpg"))
...
(defun details-form ()
  ...
  (flet ((get-detail (data key) (cadr (assoc key data :test #'string-equal))))
    (let* ((data (get-school-details))
	   ...
	   (logo-label (make-instance 'label :master *school-info-main-frame* :text "Logo"))
	   (logo-entry (make-instance 'entry :master *school-info-main-frame* :text (get-detail data "logo")))
	   (logo-file-button (make-instance 'button :master *school-info-main-frame* :text "Select logo image"
						    :command (lambda ()
...
(defun show-details ()
  ...
  (flet ((get-detail (data key) (cadr (assoc key data :test #'string-equal))))
    (let* ((data (get-school-details))
	   ...
	   (logo-label (make-instance 'label :master *school-info-main-frame* :text "Logo"))
	   (logo-data (make-instance 'label :master *school-info-main-frame* :text (get-detail data "logo"))))
      (prepare-main-window)
      (grid heading 0 0 :padx 10 :pady 5)
      (grid name-label 1 0 :padx 10 :pady 5)
