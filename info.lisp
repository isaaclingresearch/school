...
(defun show-classes ()
  ...
  (let ((index 1)
	...
	(progn
	  ...
	  (setq index (+ index 1)))
	(dolist (level levels)
	  (progn (grid (make-instance 'label :master *school-info-main-frame* :text (cadr level)) index 0)
		 (setq index (+ index 1)))
	  (let ((classes (get-classes (car level))))
	    (if (eq 0 (length classes))
		(progn (grid (make-instance 'label :master *school-info-main-frame* :text (format nil "No classes yet for level: ~a" (car level))) index 2)
		       (setq index (+ index 1)))
		(dolist (class classes)
