  ...
  (make-instance 'menubutton :master class-menu :text "Export to PDF"
		 	     :command (lambda ()
					(let* ((pdf-path (get-save-file :filetypes '(("PDF" ".pdf"))))
					       (classes (get-classes))
					       (levels (|get-level|))
					       (level-data (mapcar #'cadr levels))
					       (number-of-levels (length levels))
					       (class-data (mapcar (lambda (class)
								     "map the data to a list, with the class in the row of the level, rest are nil"
								     (let* ((datum (make-sequence 'list number-of-levels :initial-element ""))
									    (level-of-class (find (car (last class)) levels :key #'car))
									    (level-position (position level-of-class levels :test #'equal)))
								       (setf (nth level-position datum) (cadr class))
								       datum)) classes)))
					; (print class-data)
					  (export-table-to-pdf "Classes" pdf-path level-data class-data)
					  (create-menubar)
					  (grid-columnconfigure *tk* 0 :weight 1) 
					  (grid-rowconfigure *tk* 0 :weight 1)
					  (when *school-info-main-frame*
...
show the data in a treeview, with the levels as row titles and the classes of each in cells below it.
TODO: find a way to display strings with whitespace in treeview"
  (unless (null *school-info-main-frame*)
    (ltk:destroy *school-info-main-frame*))
  (setq *school-info-main-frame* (make-instance 'frame :borderwidth 5 :relief :ridge))
  (prepare-main-window)
  ...
  (let* ((levels (|get-level|))
	 (level-names (mapcar (lambda (x) (replace-all " " "--" (cadr x))) levels))
	 (tree (make-instance 'treeview :master *school-info-main-frame* :columns (format nil "{~{'~a' ~}}" level-names))))
    (dolist (level levels)
      (let ((pos (position level levels :test #'equal))
	    (class-list (make-list (length levels)))
	    (classes (get-classes (car level))))
	(treeview-heading tree (format nil "#~d" (+ 1 pos)) :text (replace-all " " "--" (cadr level)))
	(dolist (class classes)
	  (treeview-insert tree :values (setf (nth pos class-list) (replace-all " " "--" (cadr class)))))
	))
    (grid tree 0 0)))
  ...
  (let ((index 1)
	...
	(dolist (level levels)
	  ...
	  (let ((classes (get-classes (car level))))
		...
		(progn (grid (make-instance 'label :master *school-info-main-frame* :text "No classes yet.") index 1)
		       (setq index (+ index 1)))
		(dolist (class classes)
		  ...
		  (let ((streams (get-streams (car class))))
			...
			(progn (grid (make-instance 'label :master *school-info-main-frame* :text "No streams yet.") index 2)
			       (setq index (+ index 1))
			       (grid (make-instance 'separator :master *school-info-main-frame*) (+ 1 index) 1 :columnspan 2 :sticky "nsew")
			       (setq index (+ 2 index)))
			(progn (dolist (stream streams)
				 (grid (make-instance 'label :master *school-info-main-frame* :text (cadr stream)) index 2)
				 (setq index (+ 1 index)))
			       (grid (make-instance 'separator :master *school-info-main-frame*) (+ 1 index) 1 :columnspan 2 :sticky "nsew")
			       (setq index (+ 2 index)))
			)))))
	  (grid (make-instance 'separator :master *school-info-main-frame*) index 0 :columnspan 3 :sticky "nsew")
	  (setq index (+ index 1))))))
...
;;; show the class and stream, then list subjects under each.
  (let ((index 1)
	(levels (|get-level|)))
    (flet ((separate (x y span add)
	     (grid (make-instance 'separator :master *school-info-main-frame*) x y :columnspan span :sticky "nsew")
	     (setq index (+ (if add add 1) index))))
      (if (eq 0 (length levels))
	  (progn
	    (grid (make-instance 'label :master *school-info-main-frame* :text "No level has been added yet") 1 0 :padx 10 :pady 5)
	    (setq index (+ index 1)))
	    ...
	    (let ((classes (get-classes (car level))))
		  ...
		  (dolist (class classes)
		    ...
		    (let ((streams (get-streams (car class))))
		      (if (eq 0 (length streams))
			  (progn (grid (make-instance 'label :master *school-info-main-frame* :text "No streams yet.") index 2)
				 (separate (+ 1 index) 2 3 2))
			  (dolist (stream streams)
			    (grid (make-instance 'label :master *school-info-main-frame* :text (cadr stream)) index 2)
			    (setq index (+ 1 index))
			    (let ((subjects (get-subjects (car stream))))
			      (if (eq 0 (length subjects))
				  (progn  (grid (make-instance 'label :master *school-info-main-frame* :text "No subjects yet.") index 3)
					  (setq index (+ 1 index)))
				  (dolist (subject subjects)
				    (grid (make-instance 'label :master *school-info-main-frame* :text (format nil "~a - ~a" (cadr subject) (cadddr subject))) index 3)
				    (setq index (+ 1 index)))))
			    (separate index 2 2 1))))
		    (separate index 1 3 1))))
	    (separate index 0 4 1))))))
			  ...
			  (dolist (stream streams)
			    ...
			    (let ((subjects (get-subjects (car stream))))
				  ...
				  (progn  (grid (make-instance 'label :master *school-info-main-frame* :text "No subjects yet.") index 3)
					  (setq index (+ 1 index)))
				  (dolist (subject subjects)
				    (grid (make-instance 'label :master *school-info-main-frame* :text (cadr subject)) index 3)
				    (setq index (+ 1 index))
				    (let ((papers (get-papers (car subject))))
				      (if (eq 0 (length papers))
					  (progn  (grid (make-instance 'label :master *school-info-main-frame* :text "No papers yet.") index 4)
						  (setq index (+ 1 index)))
					  (dolist (paper papers)
					    (grid (make-instance 'label :master *school-info-main-frame* :text (format nil "~a - ~a" (cadr paper) (caddr paper))) index 4)
					    (setq index (+ index 1)))))
				    (separate index 3 2 1))))
			    (separate index 2 3 1))))
		    (separate index 1 4 1))))
	    (separate index 0 5 1))))))
