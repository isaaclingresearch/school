...
(defun mine-1 (&optional (file #P"~/common-lisp/school/mine.pdf"))
  (pdf:with-document ()
	...
	(let ((helvetica (pdf:get-font "Helvetica"))
	      (logo (make-jpeg-image "~/common-lisp/school/kawanda.jpeg")))
	  (pdf:add-images-to-page logo)
	  (pdf:draw-image logo 10 750 70 200 0 t)
	  (pdf:in-text-mode
	    (pdf:set-font helvetica 36.0)
					; (pdf:move-text 595 800)
	    (pdf:draw-text "")
	    (pdf:polyline '((0 740) (595 740))) ; #(0 0 595 841)
	    (pdf:stroke)
					;   (pdf:move-text 100 800)
					;  (pdf:draw-text "cl-pdf: Example 1")
					; (pdf:move-text 100 -36)
					;(pdf:draw-text "My own 1")
					;   (pdf:move-text -300 -72)
